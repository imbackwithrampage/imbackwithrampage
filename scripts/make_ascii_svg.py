...
Monochrome is deliberate -- one fill color + good density ramp + high contrast.
GitHub renders SVGs embedded via <img> and runs SMIL animations natively.
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "sunil-ascii.svg")
...
CONTRAST = 1.08
BRIGHTNESS = 1.0
GAMMA = 1.15          # >1 brightens mids -> face lands in sparser chars
SHARPEN = False
...
FRAME = "#30363d"
TITLE_TEXT = "#7d8590"
INK = "#c9d1d9"      # clean single monochrome ascii ink
CURSOR = "#c9d1d9"

# reveal timing (one-shot; cursor rasters top -> bottom)
ROW_DUR = 0.11
STAGGER = 0.11
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">sunil@github: ~$ ./portrait.sh</text>')
...
whoami_label = "sunil@github:~$ whoami "
name_val = "Sunil Sachindar S A"
parts.append(f'<line x1="0" y1="{status_line_y:.1f}" x2="{CANVAS_W}" y2="{status_line_y:.1f}" stroke="{FRAME}"/>')
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{whoami_label}<tspan fill="{INK}">{name_val}</tspan></text>')
cursor_x = PAD + 280
